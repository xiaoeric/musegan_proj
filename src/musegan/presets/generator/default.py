    ...
    def __call__(self, tensor_in, condition=None, training=None, slope=None):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope('shared'):
                ...
                h = tconv_layer(h, 256, (1, 2, 3), (1, 2, 3))        # 4, 2, 3
                h = tconv_layer(h, 128, (1, 2, 3), (1, 2, 2))        # 4, 4, 7

            # Pitch-time private network
            with tf.variable_scope('pitch_time_private'):
                s1 = [tconv_layer(h, 32, (1, 1, 12), (1, 1, 12))     # 4, 4, 84
                      for _ in range(self.n_tracks)]
                s1 = [tconv_layer(s1[i], 16, (1, 3, 1), (1, 3, 1))   # 4, 12, 84
                      for i in range(self.n_tracks)]

            # Time-pitch private network
            with tf.variable_scope('time_pitch_private'):
                s2 = [tconv_layer(h, 32, (1, 3, 1), (1, 3, 1))       # 4, 12, 7
                      for _ in range(self.n_tracks)]
                s2 = [tconv_layer(s2[i], 16, (1, 1, 12), (1, 1, 12)) # 4, 12, 84
                      for i in range(self.n_tracks)]
            ...
            with tf.variable_scope('merged_private'):
                h = [norm(tconv3d(h[i], 1, (1, 1, 1), (1, 1, 1)))    # 4, 12, 84
                     for i in range(self.n_tracks)]
                h = tf.concat(h, -1)
